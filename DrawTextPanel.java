...
//		s.setBackgroundTransparency(0.7);  // Default is 0, meaning background is not transparent.
	...
	private void doMenuCommand(String command) {
		if (command.equals("Save...")) { // save all the string info to a file
			...
			File saveFile = fileChooser.getOutputFile(this, "Select a File", "TextCollageCopy.txt");
			try {
				
				PrintWriter wrt = new PrintWriter(saveFile);
				
				wrt.println("TextCollage copy file");
				wrt.println(canvas.getBackground().getRed());
				wrt.println( canvas.getBackground().getGreen());
				wrt.println( canvas.getBackground().getBlue());
				...
				if (theString != null) {
					...
					for (DrawTextItem str : theString) {
						...
						wrt.println("String");
						wrt.println(str.getString());
						
						wrt.println(str.getTextColor().getRed());
						wrt.println(str.getTextColor().getGreen());
						wrt.println(str.getTextColor().getBlue());
						wrt.println(str.getTextTransparency());
						
						wrt.println(str.getX());
						wrt.println(str.getY());
			...
			File selectedFile = fileChooser.getInputFile(this, "Open the file.");
			
			try {
				
				Scanner rdf = new Scanner(selectedFile);
				
				if ( ! rdf.nextLine().equals("TextCollage copy file")) {
					
					return;
					
				}
				
				Color bgColor = new Color(rdf.nextInt(),rdf.nextInt(),rdf.nextInt());
				
				ArrayList<DrawTextItem> savedStr = new ArrayList<>();
				DrawTextItem saveditem;
				
				rdf.nextLine();
				
				while(rdf.hasNext() && rdf.nextLine().equals("String")) {
					
					saveditem = new DrawTextItem(rdf.nextLine());
					saveditem.setTextColor(new Color(rdf.nextInt(),rdf.nextInt(),rdf.nextInt()));
					saveditem.setTextTransparency(rdf.nextDouble());
					saveditem.setX(rdf.nextInt());
					saveditem.setY(rdf.nextInt());
					
					savedStr.add(saveditem);
					rdf.nextLine();
